...
export class WordMngPage {
    ...
    return this.dbHelper.selectJoinCatForSub().then(items => {
      ...
      for(let i=0; i<items.length; i++) {
        ...
        if(subsMap.get(sub.id) == null) {
          ...
          sub.cats = subsMap.get(sub.id).cats;
        }
  ...
  private checkLec(catDs: firebase.firestore.DocumentSnapshot, cat: Category): Promise<any> {
    ...
    return catDs.ref.collection("lecs").orderBy("num").get().then(querySnapshot => {
      ...
      return this.dbHelper.selectByCatIdForLec(cat.id).then(items => {

        let lecsPros = new Array<Promise<any>>();
        ...
        querySnapshot.forEach(lecDs => {
          ...
          if(wordCheckFlag) {
            lecsPros.push(result.then(any => {
              return this.checkWord(lecDs, cat, lec).then(any => {
                return this.dbHelper.updateLec(lec);
              });
            }));
          }
        });

        return Promise.all(lecsPros).then(any => {
          let deletePros = new Array<Promise<any>>();

          map.forEach((lec: Lecture, id: string) => {
            deletePros.push(this.dbHelper.deleteByIdForLec(id));
          });
  ...
  private checkWord(lecDs: firebase.firestore.DocumentSnapshot, cat: Category, lec: Lecture): Promise<any> {
    ...
    return lecDs.ref.collection("words").orderBy("num").get().then(querySnapshot => {
      ...
      return this.dbHelper.selectByLecIdForWord(lec.id).then(items => {
        ...
        return Promise.all(wordsPros).then(any => {
          let deletePros = new Array<Promise<any>>();

          map.forEach((word: Word, id: string) => {
            deletePros.push(this.dbHelper.deleteByIdForWord(id));
          });
